...
(defun osx-terminal-do (&rest pgm)
  "Tell Terminal to run the program PGM."
  (osascript-run-str `("
...
(defun osx-terminal-script (&rest pgm)
  "Tell Terminal to run script PGM."
  (osascript-run-str `("
tell application \"Terminal\"
  activate
  do script \"" ,@pgm "\" in window 1
end tell")))
